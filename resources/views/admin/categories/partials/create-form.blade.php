<div class="max-w-md mx-auto">
    <x-ui.card>
        <x-slot name="header">
            ...
            <p class="text-sm text-muted-foreground">Add a new category for your products.</p>
        </x-slot>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div class="space-y-4">
                <div class="grid gap-2">
                    ...
                    <x-ui.input autocomplete="off" id="name" type="text" name="name" :value="old('name')" required autofocus />
                </div>

                <div class="grid gap-2">
                    <x-ui.label for="description" value="Description (Optional)" />
                    <x-ui.textarea autocomplete="off" id="description" name="description">{{ old('description') }}</x-ui.textarea>
                </div>
            </div>
